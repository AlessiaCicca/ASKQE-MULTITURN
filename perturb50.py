...
# Per Google Colab (scaricare il file)
...
def main():
    ...
    with open(args.input_path, "r", encoding="utf-8") as f_in, \
        ...
        line_counter = 0
        # Store processed data in a list
        processed_data = []
        ...
        for line_idx, line in enumerate(f_in, start=1):
            ...
            # Save the results every 50 lines
            line_counter += 1
            if line_counter % 50 == 0:
                print(f"[INFO] Saving results after processing {line_counter} lines...")
                for item in processed_data:
                    f_out.write(json.dumps(item, ensure_ascii=False) + "\n")
                # Reset processed data after saving
                processed_data = []

        # Final save for remaining data if any
        if processed_data:
            print(f"[INFO] Saving remaining results...")
            for item in processed_data:
                f_out.write(json.dumps(item, ensure_ascii=False) + "\n")

    # If running in Google Colab, automatically download the file
